...
Also provides ``assurance_node`` — a drop-in replacement for ADK's ``@node``
that emits TOOL_CALL_STARTED / COMPLETED / FAILED around every FunctionNode
so deterministic gates and tool-backed steps appear on the same gateway
timeline as LLM agents.
"""
