...
WorkflowState, reads each agent's structured output, and makes every
routing decision explicitly and auditably in Python -- exactly as
specified ("Branching must be explicit. Do not let the LLM decide
routing. Python determines routing.").

See workflow/property_controller.py (v2) for the same business workflow
expressed as a real google.adk.workflow.Workflow graph.
...
def run_workflow(file_path: str, tracker: Optional[ProgressTracker] = None) -> Dict[str, Any]:
    ...
    with tracker.step(state.current_phase, "SubmissionIntakeAgent"):
        ...
        completeness = submission_intake_agent.run(
            applicant.raw_fields, deterministic_missing, list(MANDATORY_LABELS.values()),
    ...
    hazard_scan = detect_hazards(applicant.raw_fields, progress_callback=tracker.tool_callback(state.current_phase, "hazard_detection_tool"))
    state.hazards = hazard_scan["hazards_declared"]

    mismatch_scan = detect_mismatches(applicant.raw_fields, linked_html, progress_callback=tracker.tool_callback(state.current_phase, "mismatch_detection_tool"))

    with tracker.step(state.current_phase, "DocumentIntelligenceAgent"):
        state.agents_executed += 1
        doc_intel = document_intelligence_agent.run(
            applicant.raw_fields, hazard_scan, mismatch_scan, [d.doc_type for d in linked],
            progress_callback=tracker.agent_callback(state.current_phase),
        )
    ...
    if doc_intel["disclosure_mismatch"]:
        ...
        if human_underwriter_result["action"] == "Decline":
            ...
            return _handle_senior_escalation(
                ...
                authority_check=None, tracker=tracker,
            )
    ...
    vendor_approval = check_vendor_approval(vendor_name, progress_callback=tracker.tool_callback(state.current_phase, "vendor_approval_tool"))

    # ---- Decision 3: Is external vendor approved? ----
    tracker.gate_decision(state.current_phase, "Decision3_VendorApproved", str(vendor_approval["approved"]))
    ...
    pii_result = redact_pii(applicant.raw_fields, progress_callback=tracker.tool_callback(state.current_phase, "pii_redaction_tool"))
    # ---- Decision 4: Does payload contain PII? ---- (redact_pii always
    # returns a safe payload either way; the gate only affects the audit note)
    tracker.gate_decision(state.current_phase, "Decision4_PayloadContainsPII", str(pii_result["pii_found"]))
    state.log("PII redacted before CAT vendor call." if pii_result["pii_found"] else "No PII found in CAT vendor payload.")

    cat_result = call_cat_vendor(
        vendor_name, pii_result["redacted_payload"],
        applicant.flood_zone, applicant.earthquake_zone, applicant.cyclone_zone, applicant.wildfire_zone,
        progress_callback=tracker.tool_callback(state.current_phase, "cat_vendor_tool"),
    )
    state.cat_results = {**cat_result, "vendor_approved": True, "pii_redacted": pii_result["pii_found"]}

    with tracker.step(state.current_phase, "CATExposureAgent"):
        state.agents_executed += 1
        cat_exposure = cat_exposure_agent.run(
            vendor_approval, pii_result, cat_result,
            progress_callback=tracker.agent_callback(state.current_phase),
        )
    ...
    risk_score_result = score_property_risk(
        applicant.raw_fields, len(state.hazards), len(state.disclosure_mismatches), cat_result["cat_score"],
        applicant.previous_claims_count, progress_callback=tracker.tool_callback(state.current_phase, "property_risk_scoring_tool"),
    )
    with tracker.step(state.current_phase, "RiskSummaryAgent"):
        state.agents_executed += 1
        risk_summary = risk_summary_agent.run(
            risk_score_result, hazard_scan, mismatch_scan, cat_result,
            progress_callback=tracker.agent_callback(state.current_phase),
        )
    ...
    pricing_result = calculate_pricing(
        ...
        applicant.deductible or "", progress_callback=tracker.tool_callback(state.current_phase, "pricing_tool"),
    )
    with tracker.step(state.current_phase, "PricingAgent"):
        state.agents_executed += 1
        pricing = pricing_agent.run(
            pricing_result, risk_summary["risk_category"], risk_summary["material_risk"],
            progress_callback=tracker.agent_callback(state.current_phase),
        )
...
def _handle_override(state, application_id, applicant, doc_intel, risk_summary, pricing,
    ...
    if authority_check["exceeds_authority"]:
        ...
        return _handle_senior_escalation(
            state, application_id, applicant, doc_intel, risk_summary, pricing, human_underwriter_result,
            authority_check=authority_check, tracker=tracker,
        )
...
def _handle_senior_escalation(state, application_id, applicant, doc_intel, risk_summary, pricing,
                               human_underwriter_result, authority_check, tracker: ProgressTracker):
    state.current_phase = "PHASE_7_SENIOR_UNDERWRITER"
    with tracker.step(state.current_phase, "SeniorUnderwriterAgent"):
        state.agents_executed += 1
        state.human_reviews += 1
    ...
    state.record_lineage("SeniorUnderwriterAgent", "APPROVE" if senior_result["approve"] else "REJECT")
