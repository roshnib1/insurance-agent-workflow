...
def is_negative_or_none(value: Optional[str]) -> bool:
    """'None Reported' / 'No' / 'No - ...' / '' / None -> True."""
    if not value:
        return True
    lowered = str(value).strip().lower()
    if lowered in ("no", "none", "none reported", "n/a", "not applicable", "nil"):
        return True
    return lowered.startswith("no -") or lowered.startswith("no,") or lowered == "no."
